 ...
 * root review_items, memorization, and weak_areas. Reading/grammar practice
 ...
 *
 * lettersDue, conceptsDue, and rootsDue fold into the total the caller uses
 * to decide whether to show the "start" button, but aren't broken out as
 * their own visible row — same pattern Sub-phase 2.5 established for
 * lettersDue: these item types just need to be practiceable, not a new
 * summary line/i18n string apiece.
 */
...
/**
 * Combines due vocabulary + letter + concept + ayah + root + memorization
 ...
 * 3.2A for the same reason; "root" was added when Level 3 Batch 1 started
 * seeding root-type items from arabic-roots-intro/word-patterns — without
 * it, due root-type review items would be due but never actually reachable
 * from this page, exactly like letters before the 2.5 fix.
 */
export async function fetchPracticeQueue(userId: string, limit = 20): Promise<DailyStudyItem[]> {
  ...
  const [{ data: due }, { data: weak }] = await Promise.all([
    supabase
      ...
      .select("*")
      .eq("user_id", userId)
      .in("item_type", ["word", "ayah", "letter", "concept", "root"])
      .lte("due_date", today)
      .neq("status", "suspended")
