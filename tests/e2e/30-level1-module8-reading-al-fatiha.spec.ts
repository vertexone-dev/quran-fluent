...
test.describe("Level 1 Module 8 — Reading Al-Fatiha", () => {
  ...
  test("resume after refresh: reopening a partially-completed lesson resumes at the saved step, not the beginning", async ({
    ...
    // nothing otherwise blocks page.reload() below from racing ahead of the
    // actual network write -- invisible against local/CI's near-zero-
    // latency Supabase, but real against production's network round-trip
    // (confirmed failing in production run #21; reproduced locally by
    // injecting latency on this exact request). Waiting for each write's
    // own response before the next action is a deterministic fix tied to
    // the real operation, not a blind timeout.
    ...
    // step-0 write on mount, before either click -- waited out here first so
    // it can't be the response either click's own wait below ends up
