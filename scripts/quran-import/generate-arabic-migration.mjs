...
function applyBismillahRules(ayahs) {
  ...
  if (!shaddaVariant) throw new Error("Surah 95 ayah 1 does not carry the expected shadda-idgham Bismillah prefix");
...
async function main() {
  ...
  for (const existing of existingAyahs) {
    ...
    if (
      ...
      substantiveDifferences.push({ key, existing: existing.arabic_text, fresh: fresh.arabic_text });
    }
  }
  ...
  console.log("  0 substantive differences against existing rows — safe to generate additive INSERTs.\n");
  ...
  const missingAyahs = strippedAyahs.filter((a) => !existingAyahKeys.has(`${a.surah_number}:${a.ayah_number}`));

  console.log(`Surahs to insert: ${missingSurahs.length} (of 114; ${existingSurahNumbers.size} already present)`);
  console.log(`Ayahs to insert:  ${missingAyahs.length} (of 6236; ${existingAyahKeys.size} already present)`);
  ...
  const ayahValues = missingAyahs
    ...
    .map((a) => `(${a.surah_number}, ${a.ayah_number}, ${sqlQuote(a.arabic_text)}, ${surahSourceSubquery})`);
